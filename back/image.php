...
<form method="post" action="./api/edit.php?do=<?= $do ?>">
    ...
    <div style="text-align:center;">
        <a class="bl" style="font-size:30px;" href="?do=<?= $do ?>&p=<?= $p['prev'] ?>">&lt;&nbsp;</a>
        <?php
        for ($i = 1; $i <= $p['pages']; $i++) {
        ...
        ?>
        <a class="bl" style="font-size:30px;" href="?do=<?= $do ?>&p=<?= $p['next'] ?>">&nbsp;&gt;</a>
    </div>
    <table style="margin-top:40px; width:70%;">
        <tbody>
            <tr>
                <td width="200px"><input type="button" onclick="op('#cover','#cvr','./modal/upload.php?do=<?= $do ?>')" value="新增校園映像資料圖片"></td>
